...
@jit(nopython=True, fastmath=True, parallel=True, cache=True)
def loss_numba(alpha: np.ndarray, X:np.ndarray, Y:np.ndarray, epsilon: float, lambda2: float, beta: float) -> (float, np.ndarray):
    """
...
def owlqn(alpha: np.ndarray, X: np.ndarray, Y: np.ndarray, epsilon: float = 0.1, lambda1: float = 0,
    ...
    try: # PyLBFGS throws an error if max_iterations is exceeded, this is a workaround to convert it into a warning
        def p(x, g, fx, xnorm, gnorm, step, k, num_eval, *args):
            if k >= max_iterations:
                alpha[:] = x
        alpha = fmin_lbfgs(f=f, x0=alpha, progress=p, orthantwise_c=lambda1, max_iterations=max_iterations, line_search="wolfe" if lambda1 > 0 else "default")
